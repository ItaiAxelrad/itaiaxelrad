...
- Work with thematic (categorical maps)
...
To download the data you can clone the Github repository (<https://github.com/fyousef/UCLA> remote-sensing/tree/master/Data/Lab5) to your local computer. All the required datasets are  located within the data folder.
...
Figure 1 gives a simple demonstration of how the random forest method works in principle. For  an introduction to the random forest algorithm, see this presentation.

Fig 1. Schematic showing how the Random Forest method constructs classification trees from  random subsets of a training dataset. Each tree determines the labels assigned based on the  training dataset. Once all trees are assembled, classes are assigned to unknown pixels based on  the class which receives the majority of votes based on all the decision trees constructed.
...
## Make a new RasterBrick of covariates by adding NDVI and VCF layers covs <- addLayer(gewata, ndvi, vcfGewata)
...
names(covs) <- c("band2", "band3", "band4", "NDVI", "VCF") plot(covs)
...
## Assign 'Code' values to raster cells (where they overlap) classes <- rasterize(trainingPoly, ndvi, field='Code')

You might get warnings like In '\[<-'('\*tmp\*', cnt, value =

p@polygons\[\[i\]\]@Polygons\[\[j\]\]) : implicit list embedding of S4 objects is deprecated. You can ignore these for now.
...
## Combine this new brick with the classes layer to make our input training dat aset
...
points(band4 ~ band3, data = val_wetland, pch = ".", col = "light blue") legend("topright", legend=c("cropland", "forest", "wetland"), fill=c("orange", "dark green", "light blue"), bg="white")
...
We will use the same layers (from the covs rasterBrick) as in the Random Forest classification for  this classification exercise. As before, we need to extract all values into a data.frame. But this  time we will extract all values, since we are not limited to a training dataset. valuetable <- getValues(covs)
...
First, we will insert these values into the original valuetable data.frame. ## Convert valuetable to a data.frame
...
## Make an NA-value raster based on the LC raster attributes formask <- setValues(raster(predLC), NA)

## Assign 1 to formask to all cells corresponding to the forest class formask\[predLC\==2\] <- 1
...
## Group raster cells into clumps based on the Queen's Case if(!file.exists(fn <- "data/clumformask.grd")) {
...
## which rows of the data.frame are only represented by one cell? str(which(clumpFreq$count\==1))
...
## Assign NA to all clumps whose IDs are found in excludeID formaskSieve\[forestclumps %in% excludeID\] <- NA

## Zoom in to a small extent to check the results

# Note: you can define your own zoom by using e <- drawExtent() e <- extent(c(811744.8, 812764.3, 849997.8, 850920.3))
...
This data.frame represents a lookup table for the raster we just loaded. The $ID column  corresponds to the values taken on by the lulc raster, and the $Class column describes the
...
We learned about how a model object in R can be used to predict raster values. This is a good  example of how we can use the strengths of R's statistical packages in a spatial context. Of course, there are many more types of model objects than what is returned by
